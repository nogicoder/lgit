...
from lgit_stash import LgitStash
...
def remove(fname):
    ...
    if fname in added_files and os.path.exists(fname):
        os.unlink(fname)
...
def main():
    ...
    if not Initialize.checkParents() and command != 'init':
        ...
        if command == 'init':
            ...
            LgitBranch(command, branch_name)
        elif command == 'checkout':
            branch_name = args.checkout()
            LgitBranch(command, branch_name)
        elif command == 'merge':
            branch_name = args.merge()
            LgitBranch(command, branch_name)
        elif command == 'stash':
            flag = args.stash()
            if flag != 'apply':
